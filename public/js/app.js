...
function loadFormData() {
  var storedData = localStorage.getItem('forms');
  if (storedData) {
    try {
      var _localStorage$getItem2;
      var forms = JSON.parse(storedData);
      var id = (_localStorage$getItem2 = localStorage.getItem('atual')) !== null && _localStorage$getItem2 !== void 0 ? _localStorage$getItem2 : 0;
      if (forms[id]) {
        forms[id].dados.forEach(function (estadoData) {
...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  if (document.querySelector('.lista-automatos')) {
    var forms = JSON.parse(localStorage.getItem('forms'));
    if (forms) {
      var container = document.querySelector('.lista-automatos');
  ...
  var swiper = new Swiper('.swiper-equipe', {
    slidesPerView: 3,
    spaceBetween: 32,
    loop: false
  });
